...
namespace Repositories.Queries
{
    public static class PredictionQueries
    {
        ...
        {
            return $"SELECT cp.competition_id, p.id AS prediction_id, p.name, p.points FROM Prediction p JOIN Competition_Prediction cp ON cp.prediction_id = p.id JOIN User_Prediction up ON up.prediction_id = p.id WHERE up.[user_id] = {u.ID}";
        }

        internal static string GetPredictionComponents(int prediction_id)
        {
            return $"SELECT pd.ID, driver_id, position, checked, correct FROM Prediction_Driver pd JOIN Prediction p ON p.id = pd.prediction_id WHERE p.id = {prediction_id}";
        }
    }
}
